...
final class RpcMethods {
    ...
    static {
        ...
        // 1.21.1 的 ByteBufCodecs 无 LONG_ARRAY（26.1 新增），手写等价编解码：VarInt 长度 + VarLong 元素
        DEFAULTS.put(long[].class, StreamCodec.of(
            (ByteBuf buf, long[] arr) -> {
                VarInt.write(buf, arr.length);
                for (long v : arr) VarLong.write(buf, v);
            },
            buf -> {
                long[] arr = new long[VarInt.read(buf)];
                for (int i = 0; i < arr.length; i++) arr[i] = VarLong.read(buf);
                return arr;
            }
        ));
    ...
    static Method resolve(String className, String methodName, String descriptor) {
        return METHOD_CACHE.computeIfAbsent(className + "#" + methodName + descriptor, ignored -> doResolve(className, methodName, descriptor));
    }
