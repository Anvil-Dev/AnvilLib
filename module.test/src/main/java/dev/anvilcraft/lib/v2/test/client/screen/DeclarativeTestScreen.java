...
public class DeclarativeTestScreen extends DeclarativeScreen {
    ...
    protected void content(UIScope scope) {
        ...
        Ref<@NonNull Integer> counter = comp.ref(0);
        Ref<@NonNull Boolean> checked = comp.ref(false);
        Ref<@NonNull String> text = comp.ref("");

        scope.Scrollable(
            this.height, scroll -> scroll.Column(col -> {
                // ── 标题 ──
                ...
                col.Dropdown(options, 0, 80).onChange(_ -> {
                });
                ...
                col.Row(row -> {
                    row.Button("Styled").modifier(Modifier.NONE.background(0xFF884444).border(1, 0xFFFF8888));
                    row.Text("  ");
