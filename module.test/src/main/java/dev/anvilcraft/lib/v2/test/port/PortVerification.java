...
public final class PortVerification {
    ...
    private static void geometry() {
        ...
        for (int xRot = 0; xRot < 16; xRot++) {
            for (int yRot = 0; yRot < 4; yRot++) {
                var symmetry = com.mojang.math.OctahedralGroup.values()[(xRot % 12) * 4 + yRot];
                Matrix4f basis = new Matrix4f();
                // 1.21.4 的 SymmetricGroup3 矩阵保留了单位对角项，使用方向映射构造正确的正交基。
                var axes = new net.minecraft.core.Direction[]{net.minecraft.core.Direction.EAST, net.minecraft.core.Direction.UP, net.minecraft.core.Direction.SOUTH};
                for (int axis = 0; axis < 3; axis++) {
                    var direction = symmetry.rotate(axes[axis]);
                    basis.setColumn(axis, new org.joml.Vector4f(direction.getStepX(), direction.getStepY(), direction.getStepZ(), 0));
                }
                if (xRot >= 12) basis = new Matrix4f(net.minecraft.client.resources.model.BlockModelRotation.values()[(xRot - 12) * 4 + yRot].getRotation().getMatrix());
                Matrix4f transform = new Matrix4f().translation(0.5F, 0.5F, 0.5F)
                    .mul(basis)
                    .translate(-0.5F, -0.5F, -0.5F);
                check(transform.isAffine() && Math.abs(transform.determinant()) > 1.0E-9, "原版变换无效: " + xRot + "," + yRot + " " + transform);
                ...
                for (double height : new double[]{1.62, 1.27}) {
                    for (int x = 0; x <= 40; x++) for (int z = 0; z <= 40; z++) {
                        double localX = xRot < 12 ? x / 40.0 : (x + 0.5) / 41.0;
                        double localZ = xRot < 12 ? z / 40.0 : (z + 0.5) / 41.0;
                        Vec3 start = transform(transform, new Vec3(localX, 1 + height, localZ));
                        Vec3 end = transform(transform, new Vec3(localX, -1, localZ));
                        var hit = part.clip(start, end);
                        check(hit != null, "旋转立方体顶面漏选: " + xRot + "," + yRot + " @ " + x + "," + z);
                        Vec3 actual = start.lerp(end, hit.fraction());
                        Vec3 expected = transform(transform, new Vec3(localX, 1, localZ));
        ...
        SimpleWeightedRandomList<ModelSelection> vanilla = SimpleWeightedRandomList.<ModelSelection>builder().add(first, 2).add(second, 5).build();
        for (int seed = 0; seed < 4096; seed++) {
            ...
            vanilla.getRandomValue(expected).orElseThrow().collect(expected, reference);
    ...
    private static void recipePruning() throws Exception {
        ...
        var vanilla = net.minecraft.world.item.crafting.RecipeMap.create(List.of(registered.getFirst()));
        ((dev.anvilcraft.lib.v2.recipe.injection.IRecipeMapExtension) vanilla).anvillib$addRecipes(registered);
        check(vanilla.values().size() == 100 && vanilla.byKey(registered.getLast().id()) == registered.getLast(), "目标版配方容器保留 ResourceKey 索引并去重");
    ...
    private static void persistence() {
        ...
        var tag = (net.minecraft.nbt.CompoundTag) state.toTag(registries);
        check(MultiblockState.fromTag(tag, registries).isFormed(), "多方块形成状态持久化");
    }
