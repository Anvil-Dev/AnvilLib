...
public final class PortVerification {
        ...
        for (int xRot = 0; xRot < 12; xRot++) {
            for (int yRot = 0; yRot < 4; yRot++) {
                Matrix4f transform = new Matrix4f().translation(0.5F, 0.5F, 0.5F)
                    .mul(net.minecraft.client.renderer.block.dispatch.BlockModelRotation.get(
                        com.mojang.math.OctahedralGroup.values()[xRot * 4 + yRot]).transformation().getMatrix())
    ...
    private static void variants() {
        ...
        Identifier tab = Identifier.fromNamespaceAndPath("anvillib_test", "port_sections");
        ...
        check(CreativeTabSections.arrange(Identifier.withDefaultNamespace("plain"), output) == output, "普通创造栏内容保留");
        output.clear();
        CreativeTabSections.build(tab, parameters, (stack, visibility) -> output.add(stack), sections -> {
    ...
    private static InWorldRecipe recipe(IRecipeTrigger trigger, List<IRecipePredicate<?>> predicates, int priority) {
        return new InWorldRecipe(new ItemStackTemplate(Items.STONE), trigger, List.of(), predicates, List.of(), priority, true, 1);
    }
    ...
    private static void recipePruning() throws Exception {
        IRecipeTrigger trigger = new IRecipeTrigger.Impl(Identifier.withDefaultNamespace("port"));
        ...
        for (int i = 0; i < 100; i++) {
            var holder = new RecipeHolder<>(ResourceKey.create(Registries.RECIPE, Identifier.withDefaultNamespace("port_" + i)),
        ...
        holders.clear();
        var direct = new RecipeHolder<>(ResourceKey.create(Registries.RECIPE, Identifier.withDefaultNamespace("direct")), recipe(trigger, List.of(dirt), 1));
        holders.add(direct);
    ...
    private static void itemCache() {
        ...
        handler.set(0, ItemResource.of(Items.STONE), 8);
        ItemResourceHandlerCacheElement cache = new ItemResourceHandlerCacheElement(new ItemCache(null), handler, 0, Vec3.ZERO, Vec3.ZERO);
        cache.sync();
        check(handler.getAmountAsInt(0) == 8, "未变化输出槽不能丢物品");
        cache.shrink(3);
        cache.sync();
        check(handler.getAmountAsInt(0) == 5, "拒绝插入的槽位只提取消耗差量");
    }

    private static void persistence() {
        var state = new MultiblockState(BlockPos.ZERO, ResourceKey.create(LibRegistries.DEFINITIONS_KEY, Identifier.withDefaultNamespace("port")), true);
