...
public final class ModelSelections {
    ...
    private @Nullable ModelSelection resolve(BakedModel model, BlockState state, int depth) {
        ...
        if (model instanceof WeightedModelAccessor weighted) {
            ...
            AABB bounds = null;
            for (WeightedEntry.Wrapper<BakedModel> entry : weighted.anvillib_cube$list().unwrap()) {
        ...
        if (model instanceof MultipartModelAccessor multipart) {
            ...
            for (net.minecraft.client.resources.model.MultiPartBakedModel.Selector entry : multipart.anvillib_cube$selectors()) {
                if (!entry.condition().test(state)) continue;
                ModelSelection part = this.resolve(entry.model(), state, depth + 1);
