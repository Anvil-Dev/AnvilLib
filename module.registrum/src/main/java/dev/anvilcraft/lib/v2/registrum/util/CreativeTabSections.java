...
public final class CreativeTabSections implements CreativeModeTab.Output {
    ...
    private static final Map<Identifier, Layout> LAYOUTS = new ConcurrentHashMap<>();
    private static final Map<Identifier, List<PlacedSection>> PLACEMENTS = new ConcurrentHashMap<>();
    ...
    public static Collection<ItemStack> arrange(Identifier tabId, Collection<ItemStack> items) {
        return arrange(tabId, items, false);
    }

    /** 按原始分区边界分别折叠变体，避免前一区缩短后吞掉后续横幅。 */
    public static Collection<ItemStack> arrange(Identifier tabId, Collection<ItemStack> items, boolean foldVariants) {
    ...
    public static List<PlacedSection> placedSections(Identifier tabId) {
