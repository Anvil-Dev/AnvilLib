...
> ⚠️ 状态更新（2026-08-12）：`module.rendering` 与 `renderdoc-loader` 已从 `port/1.21.1` 分支移除（构建依赖、CI 任务与文档引用已同步清理）。本文为 2026-08-10 的快照分析，正文中 dev/26.1 侧的模块描述仍为历史事实。
...
| 模块                           | 结论                            | 关键依据                                                                                                                                                                                                               | 回迁项         |
|--------------------------------|---------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|
| `module.codec`                 | **dev 超集**                    | port 侧 `0b56109`/`50aeac7`/`6a0e8a9` 均有 dev 同主题提交（`3b5dfea`/`2e1b537`/`922d523`）；dev 另有 `52c3f8b`/`cf75837`（PositionPredicate AT 补充）、`3b5dfea` 等独有内容 | `173a454`（portable Codec/MapCodec 方法，2026-08-11 已回移植，落点待提交） |
