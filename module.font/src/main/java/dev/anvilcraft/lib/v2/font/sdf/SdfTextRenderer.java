...
public final class SdfTextRenderer {
    ...
    private void drawStringWithAtlas(
        ...
        for (SdfTextLayout.PageQuads pq : layout.pages()) {
            drawAtlasPipeline(graphics, pq, color, x, y);
        }
    }
    ...
    private static void drawAtlasPipeline(
        ...
        SdfTextRenderState state = new SdfTextRenderState(
            ...
            originX,
            originY,
            dev.anvilcraft.lib.v2.rendering.state.GuiRenderAccess.scissor(graphics)
        );
        dev.anvilcraft.lib.v2.rendering.state.GuiRenderAccess.submit(graphics, state);
    }
}
