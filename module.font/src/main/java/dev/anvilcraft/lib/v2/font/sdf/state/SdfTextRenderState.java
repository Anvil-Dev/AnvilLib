...
public record SdfTextRenderState(
    ...
    public void executeDrawAfterSetPipeline(RenderPass renderPass) {
        AbstractTexture texture = Minecraft.getInstance().getTextureManager().getTexture(this.atlasTexture);
        renderPass.bindSampler("DiffuseSampler", texture.getTextureView());
    }
    ...
    public void buildVertices(VertexConsumer consumer, float z) {
        ...
        for (SdfTextLayout.GlyphQuad quad : this.glyphs) {
            ...
            // Vertex 0: top-left
            consumer.addVertexWith2DPose(this.pose, x0, y0, z).setColor(this.color)
                .setUv(u0, v0);

            // Vertex 1: bottom-left
            consumer.addVertexWith2DPose(this.pose, x0, y1, z).setColor(this.color)
                .setUv(u0, v1);

            // Vertex 2: bottom-right
            consumer.addVertexWith2DPose(this.pose, x1, y1, z).setColor(this.color)
                .setUv(u1, v1);

            // Vertex 3: top-right
            consumer.addVertexWith2DPose(this.pose, x1, y0, z).setColor(this.color)
                .setUv(u1, v0);
        }
    }
